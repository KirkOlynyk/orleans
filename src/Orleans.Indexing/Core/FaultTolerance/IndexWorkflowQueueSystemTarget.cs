...
namespace Orleans.Indexing
{
    ...
    /// has one or more IndexWorkflowQueueSystemTarget system-targets for each grain class, up to the number of hardware threads. A system-target is a grain that
    /// belongs to a specific silo.
    /// + Each of these system-targets has a queue of workflowRecords, which describe updates that must be propagated to indexes.Each workflowRecord contains
    ///   the following information:
    ///    - workflowID: grainID + a sequence number
    ///    - memberUpdates: the updated values of indexed fields
    ///  
    ///   Ordinarily, these workflowRecords are for grains that are active on IndexWorkflowQueueSystemTarget's silo. (This may not be true for short periods when
    ///   a grain migrates to another silo or after the silo recovers from failure).
    /// 
    /// + The IndexWorkflowQueueSystemTarget grain Q has a dictionary updatesOnWait is an in-memory dictionary that maps each grain G to the workflowRecords for G
    ...
    internal class IndexWorkflowQueueSystemTarget : SystemTarget, IIndexWorkflowQueue
    {
        ...
        public Task AddToQueue(Immutable<IndexWorkflowRecord> workflowRecord) => _base.AddToQueue(workflowRecord);

        public Task<Immutable<List<IndexWorkflowRecord>>> GetRemainingWorkflowsIn(HashSet<Guid> activeWorkflowsSet) =>_base.GetRemainingWorkflowsIn(activeWorkflowsSet);

        public Task<Immutable<IndexWorkflowRecordNode>> GiveMoreWorkflowsOrSetAsIdle() =>_base.GiveMoreWorkflowsOrSetAsIdle();

        public Task RemoveAllFromQueue(Immutable<List<IndexWorkflowRecord>> workflowRecords) => _base.RemoveAllFromQueue(workflowRecords);
